  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((r) => {
          const badge = typeClasses[(r?.type || '').toLowerCase()] || typeClasses.other;
          return (
            <div key={r._id} className="group rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                <a href={r.url || '#'} target="_blank" rel="noreferrer" className="px-3 py-2 bg-indigo-600 text-white text-sm rounded">Open</a>
                <Link to={`/resources/${r._id}/edit`} className="px-3 py-2 bg-gray-100 text-sm rounded">Edit</Link>
                <button onClick={() => onDelete(r._id)} className="px-3 py-2 bg-red-100 text-red-700 text-sm rounded">Delete</button>
              </div>
            </div>
          );
        })}
